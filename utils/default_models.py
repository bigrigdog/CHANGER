    ...
    with DownloadProgressBar(
        ...
        try:
            urllib.request.urlretrieve(
                url, filename=target, reporthook=t.update_to)
        except HTTPError:
            return
...
def ensure_default_models(models_dir: Path):
    ...
    for model_name, (url, size) in default_models.items():
        ...
        if target_path.exists():
            if target_path.stat().st_size != size:
                print(
                    f"File {target_path} is not of expected size, redownloading...")
            else:
                continue
    ...
    for thread, target_path, size in jobs:
        ...
        assert target_path.exists() and target_path.stat().st_size == size, (
            f"Download for {
                target_path.name} failed. You may download models manually instead.\n"
