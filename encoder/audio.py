...
try:
    ...
    warn(
        "Unable to import 'webrtcvad'. This package enables noise removal and is highly recommended."
    )
    webrtcvad = None
...
def preprocess_wav(
    ...
    if normalize:
        wav = normalize_volume(
            wav, target_dBFS=audio_norm_target_dBFS, increase_only=True
        )
...
def wav_to_mel_spectrogram(
    wav, sampling_rate=22050, n_fft=2048, hop_length=512, n_mels=128, min_length=128
):
    """
    ...
    if len(wav) < hop_length:
        raise ValueError(
            "Input waveform is too short for the specified hop_length.")

    # Ensure the waveform length is a multiple of hop_length to maintain consistency
    wav = wav[: len(wav) - (len(wav) % hop_length)]

    spectrogram = librosa.feature.melspectrogram(
        y=wav, sr=sampling_rate, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels
    )
    ...
    if padding_needed > 0:
        spectrogram = np.pad(
            spectrogram, ((0, 0), (0, padding_needed)), mode="constant"
        )
...
def trim_long_silences(wav):
    ...
    pcm_wave = struct.pack(
        f"{len(wav)}h", *(np.round(wav * int16_max).astype(np.int16))
    )
    ...
    voice_flags = [
        vad.is_speech(
            pcm_wave[i * 2 *
                     samples_per_window: (i + 1) * 2 * samples_per_window],
            sample_rate=sampling_rate,
        )
        for i in range(len(wav) // samples_per_window)
    ]

    voice_flags = np.array(voice_flags, dtype=np.bool_)
    smoothed_voice_flags = binary_dilation(
        voice_flags, np.ones(vad_max_silence_length + 1)
    )
