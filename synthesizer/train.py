...
def train(
    ...
    for i, session in enumerate(hparams.tts_schedule):
        ...
        data_loader = DataLoader(
            dataset, batch_size, shuffle=True, num_workers=2, collate_fn=collate_fn
        )
        ...
        for epoch in range(1, epochs + 1):
            for i, (texts, mels, embeds, idx) in enumerate(data_loader, 1):
                ...
                msg = (
                    f"| Epoch: {
                        epoch}/{epochs} ({i}/{steps_per_epoch}) | Loss: {loss_window.average:#.4} | "
                ...
                if backup_every != 0 and step % backup_every == 0:
                    backup_fpath = weights_fpath.parent / \
                        f"synthesizer_{k:06d}.pt"
                ...
                epoch_eval = (
                    hparams.tts_eval_interval == -1 and i == steps_per_epoch
                )  # If epoch is done
                step_eval = (
                    hparams.tts_eval_interval > 0
                    and step % hparams.tts_eval_interval == 0
                )  # Every N steps
...
def eval_model(
    ...
    attention_path = str(
        plot_dir.joinpath(
            "attention_step_{}_sample_{}".format(step, sample_num))
    )
